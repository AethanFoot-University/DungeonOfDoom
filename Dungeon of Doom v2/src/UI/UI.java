...
public class UI {
	...
	public void draw() {
		for (Button b: buttonList) {
			drawQuadTexRot(b.getTexture(), b.getX(), b.getY(), b.getWidth(), b.getHeight(), 0);
		}
		for (Menu m: menuList) {
			m.draw();
		}
	...
	public class Menu {
		...
		int x, y, width, height, buttonAmount, optionsWidth, optionsHeight, padding;
		...
		public Menu(String name, int x, int y, int width, int height, int optionsWidth, int optionsHeight) {
			...
			padding = (width - (optionsWidth * TILE_SIZE)) / (optionsWidth + 1);
			buttonAmount = 0;
